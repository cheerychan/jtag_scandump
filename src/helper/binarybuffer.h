...
{
	//LOG_DEBUG("[ScanDump_JTAG_Read_RegisterScan (addr_buf data): (0x%x%x)]", _buffer[1], _buffer[0]);
	//LOG_DEBUG("[ScanDump_JTAG_Read_RegisterScan (addr_buf data): (0x%p)]", _buffer);
        const uint8_t *buffer = _buffer;
        //LOG_DEBUG("[ScanDump_JTAG_Read_RegisterScan (addr_buf data): (0x%x%x)]", buffer[1], buffer[0]);
	if ((num == 32) && (first == 0)) {
	...
	unsigned first, unsigned num)
{
	...
	else if ((num == 128) && (first == 64)) {
		return 0 + ((((uint64_t)buffer[15]) << 56) |   /* Note - zero plus is to avoid a checkpatch bug */
	...
	else if ((num == 192) && (first == 128)) {
		return 0 + ((((uint64_t)buffer[23]) << 56) |   /* Note - zero plus is to avoid a checkpatch bug */
				(((uint64_t)buffer[22]) << 48) |
				(((uint64_t)buffer[21]) << 40) |
				(((uint64_t)buffer[20]) << 32) |
				(((uint64_t)buffer[19]) << 24) |
				(((uint64_t)buffer[18]) << 16) |
				(((uint64_t)buffer[17]) << 8)  |
