...
static int jtag_examine_chain(void)
{
	...
	memset(idcode_buffer,0,max_taps * 4);
	...
	if (retval != ERROR_OK)
		goto out;
	LOG_DEBUG("[Coming Debug] idcode_buffer[0] = (0x%x)", *idcode_buffer);
	LOG_DEBUG("[Coming Debug] idcode_buffer[1] = (0x%x)", *(idcode_buffer+1));
	LOG_DEBUG("[Coming Debug] idcode_buffer[2] = (0x%x)", *(idcode_buffer+2));
	LOG_DEBUG("[Coming Debug] idcode_buffer[3] = (0x%x)", *(idcode_buffer+3));
	
	if (!jtag_examine_chain_check(idcode_buffer, max_taps)) {
		retval = ERROR_JTAG_INIT_FAILED;
		goto out; //Coming add tmp
	}

	//Coming debug: the following part is not excuted, because the above is false.
...
////LOG_DEBUG("[Coming Debug] 1. ir_test = (0x%x)", *ir_test);
	
	field.num_bits = total_ir_length;
	field.out_value = ir_test;
	field.in_value = ir_test;

	//Coming comment it for debug use 2017.5.11
	...
	LOG_DEBUG("[Coming Debug] 2. ir_test = (0x%x)", *ir_test);
	LOG_DEBUG("[Coming Debug] field.in_value = (0x%x)", *(field.in_value));
	LOG_DEBUG("[Coming Debug] field.out_value = (0x%x)", *(field.out_value));
	...
	for (;; ) {
		 ...
		 */
		LOG_DEBUG("[Coming Debug] tap->ir_length = (0x%x)", tap->ir_length);
		LOG_DEBUG("[Coming Debug] tap->ir_capture_mask = (0x%x)", tap->ir_capture_mask);
		LOG_DEBUG("[Coming Debug] tap->ir_capture_value = (0x%x)", tap->ir_capture_value);
		 ...
		 */
//Coming add for debug, this system is not stable yet.
#if 1
		val = buf_get_u64(ir_test, chain_pos, tap->ir_length);
		////LOG_DEBUG("[Coming Debug] val = (0x%x)" PRIx64, val);
...
int jtag_init_inner(struct command_context *cmd_ctx)
{
	...
	int num_tap = jtag_tap_count();
	LOG_DEBUG("[Coming Debug] TAP NUMS = (0x%x)", num_tap);
