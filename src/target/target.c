...
extern struct target_type zx_pmp_target;
		...
		struct target *target)
{
	...
	if (type->mmu) {
		...
		type->mmu = no_mmu;
		/*Coming add comment for zx_riscv use 2017.01.18*/
...
static int target_write_buffer_default(struct target *target, uint32_t address, uint32_t count, const uint8_t *buffer)
{
	...
	for (size = 1; size < 4 && count >= size * 2 + (address & size); size *= 2) {
		LOG_DEBUG("[Coming] step 1");
		if (address & size) {
			int retval = target_write_phys_memory(target, address, size, 1, buffer);
			if (retval != ERROR_OK)
				return retval;
			address += size;
			count -= size;
	...
	for (; size > 0; size /= 2) {
		LOG_DEBUG("[Coming] step 2");
		uint32_t aligned = count - count % size;
		if (aligned > 0) {
			int retval = target_write_phys_memory(target, address, size, aligned / size, buffer);
			for(int i = 0; i < 100; i++)
				////LOG_DEBUG("[Coming] step 2 -- Address:[0x%x] = [0x%x]", address + i * 2, *(buffer+i));
			if (retval != ERROR_OK)
				return retval;
			address += aligned;
			count -= aligned;
...
/**************************Coming add for RISC-V debug R/W test***************************/

#if 1
COMMAND_HANDLER(handle_rd_command)
{
	if (CMD_ARGC < 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	int retval = fn(target, address, size, count, buffer);
	if (ERROR_OK == retval)
		handle_md_output(CMD_CTX, target, address, 4, 1, buffer);
...
#if 1
COMMAND_HANDLER(handle_rw_command)
{
	if (CMD_ARGC < 2)
		return ERROR_COMMAND_SYNTAX_ERROR;
	//bool physical = strcmp(CMD_ARGV[0], "phys") == 0;
...
static int handle_bp_command_list(struct command_context *cmd_ctx)
{
	...
	while (breakpoint) {
		if (breakpoint->type == BKPT_SOFT) {
			...
			command_print(cmd_ctx, "IVA-SW breakpoint: 0x%8.8" PRIx32 ", 0x%x, %i, 0x%s",
					breakpoint->address,
					breakpoint->length,
					breakpoint->set, buf);
			free(buf);
			...
			if ((breakpoint->address == 0) && (breakpoint->asid != 0))
				...
				command_print(cmd_ctx, "IVA-HW Breakpoint: 0x%8.8" PRIx32 ", 0x%x, %i",
							breakpoint->address,
							breakpoint->length, breakpoint->set);
		}
...
COMMAND_HANDLER(handle_bp_command)
{
	...
	switch (CMD_ARGC) {
		...
		case 3:
			...
			else if (strcmp(CMD_ARGV[2], "sw") == 0) {
				hw = BKPT_HSOFT;	//The flag new SW bkpts
				COMMAND_PARSE_NUMBER(u32, CMD_ARGV[0], addr);
				COMMAND_PARSE_NUMBER(u32, CMD_ARGV[1], length);
				asid = 0;
				return handle_bp_command_set(CMD_CTX, addr, asid, length, hw);
...
static const struct command_registration target_exec_command_handlers[] = {
	...
	{
		.name = "rdw",					//coming add for RISC-V debug R/W test
		.handler = handle_rd_command,
		.mode = COMMAND_EXEC,
		.help = "display memory words",
		.usage = "debug offset [count]",
	},
	{
		.name = "rww",					//coming add for RISC-V debug R/W test
		.handler = handle_rw_command,
		.mode = COMMAND_EXEC,
		.help = "display memory words",
		.usage = "debug offset [count]",
	},
